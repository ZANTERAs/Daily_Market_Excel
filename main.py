...
- Same behavior as original script, plus:
  * Saves a single Excel workbook with multiple sheets:
      - RawNews
      - MappedScored
      - DailySignals
"""
...
# with open("aliases.json", "r", encoding="utf-8") as f:
#     TICKER_ALIASES = json.load(f)
#     DEFAULT_TICKERS = list(TICKER_ALIASES.keys())
#     DEFAULT_TICKERS.append("MARKET")  # special ticker for general market news
...
# Excel writer
# -----------------------------
def save_to_excel(ts: str, news: pd.DataFrame, scored: pd.DataFrame, daily: pd.DataFrame) -> str:
    """
    Writes the three key DataFrames to a single Excel workbook with multiple sheets.
    Returns the path of the created file.
    """
    xlsx_path = os.path.join(DATA_DIR, f"news_outputs_{ts}.xlsx")
    # Try xlsxwriter first; fall back to openpyxl if not present.
    engine = None
    for candidate in ("xlsxwriter", "openpyxl"):
        try:
            with pd.ExcelWriter(xlsx_path, engine=candidate) as writer:
                news.to_excel(writer, sheet_name="RawNews", index=False)
                scored.to_excel(writer, sheet_name="MappedScored", index=False)
                daily.to_excel(writer, sheet_name="DailySignals", index=False)
            engine = candidate
            break
        except Exception as ex:
            # Remove partial file if creation failed with this engine
            try:
            ...
            continue
    if engine is None:
        print("[warn] Could not write Excel. Install 'xlsxwriter' or 'openpyxl'.")
    else:
        print(f"[info] Excel saved with engine '{engine}': {xlsx_path}")
    return xlsx_path
...
def run(tickers: List[str], backend: str, days: int, plot: bool, lookahead: int):
    ...
    # NEW: save to Excel (multi-sheet)
    xlsx_path = save_to_excel(ts, news, scored, daily)
...
def parse_args():
    ...
    # parse_known_args to be notebook-friendly (ignores -f from Jupyter)
